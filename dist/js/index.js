...
define('httpclient', ['jquery'], function ($) {
...
define('user', [
    ...
    function users() {
        ...
        http.post('userdata', { username: window.localStorage.getItem('username') }).then(function (res) {
            console.log(res);
            if (res.status === true) {
...
(function (e) {
    ...
    var d = $, h = d(window), c = d('<div class="c-float-popWrap msgMode hide">' + '<div class="weui_mask_transparent"></div>' + '<div class="c-float-modePop">' + '<div class="warnMsg"></div>' + '<div class="content"></div>' + '<div class="doBtn">' + '<button class="cancel">取消</button>' + '<button class="ok">确定</button>' + '</div>' + '</div>' + '</div>'), m = c.find('.warnMsg'), n = c.find('.content'), o = c.find('.doBtn .ok'), p = c.find('.doBtn .cancel'), j = !1, f;
    d.extend(e.prototype, {
        _init: function () {
            var a = this, b = a._options, g = b.mode, k = b.text, e = b.content, f = b.callback, l = b.background, t = b.btns, b = b.useTap ? 'tap' : 'click', i = c.attr('class'), i = i.replace(/(msg|alert|confirm)Mode/i, g + 'Mode');
            c.attr('class', i);
            ...
            j || (j = !0, d('body').append(c), h.on('resize', function () {
                setTimeout(function () {
                    a._pos();
                }, 500);
            }));
        ...
        _pos: function () {
            var a = document, b = a.documentElement, g = a.body, e, d, f;
        ...
        show: function () {
            ...
            a.isShow() ? a._cbShow() : (c.css('opacity', '0').removeClass('hide'), a._pos(), setTimeout(function () {
                ...
                c.animate({ opacity: '1' }, 300, 'linear');
            }, 1));
        },
        _cbHide: function () {
        ...
        hide: function () {
            ...
            a.isHide() ? a._cbHide() : (c.css('opacity', '1').removeClass('show'), setTimeout(function () {
                ...
                c.animate({ opacity: '0' }, 300, 'linear');
            }, 1));
        },
        flash: function (a) {
            var b = this;
...
define('shop', [
    ...
    function shop() {
        ...
        http.post('goodslist').then(function (res) {
            if (res.status == true) {
                ...
                var car = datas1.map(function (item) {
                    return '<li data-id=' + item._id + '>' + '<img src="' + item.SmallPic + '"/>' + '<div class="txt">' + '<h2>' + item.CommodityName + '</h2>' + '<p class="red">\uFFE5<b>' + item.OriginalPrice + '</b></p>' + '<a href="javasrcipt:;" class="goods-btn"></a>' + '</div>' + '</li>';
                });
                $('.j-goods').html(car);
        ...
        if (window.localStorage.getItem('token')) {
            ...
            $('.shop-nogoods').css({ 'marginTop': '10px' });
            close();
            if (window.localStorage.getItem('goodslist') == '[]') {
                nogoods();
            }
        ...
        function datas(data) {
            var car = data.map(function (item) {
                return '<div class="one" data-guid="' + item._id + '" data-pirce="' + item.qty * item.SellPrice + '">' + '<div class="check"><i class="checkI"></i></div>' + '<div class="img"><img src="' + item.SmallPic + '"></div>' + '<div class="text">' + '<h2 class="elli2">' + item.CommodityName + '</h2>' + '<div class="tag"></div>' + '<p><strong class="red">\uFFE5<b>' + item.qty * item.SellPrice + '</b></strong></p>' + '</div>' + '<div class="del"><i></i></div>' + '<div class="num">' + '<span class="cut j-click"><i></i></span>' + '<span class="input line-bottom">' + item.qty + '</span>' + '<span class="add j-click"><i></i></span>' + '</div>';
                '</div>';
            });
            return car;
        }
        function nogoods() {
            $('.shop-nogoods').html('<div class="nogoods-icon"></div>' + '<p>购物车空空的\uFF0C快去逛逛吧\uFF01</p>' + '<div class="btn">' + '<a>去逛逛</a>' + '</div>');
        }
    }
    return shop;
});
...
define('main', [
    ...
    return function () {
        $(window).scroll(function () {
            if (scrollY > 600) {
                $('.jumpTop').css({ 'display': 'block' });
            } else {
                $('.jumpTop').css({ 'display': 'none' });
            }
        });
        http.post('indx_goods').then(function (res) {
            for (var i = 0; i < 5; i++) {
                ...
                var lis = '<li>' + '<img src="https:' + src + '" alt="">' + '</li>';
                ...
                for (var j = 0; j < ii_arr.length - 1; j++) {
                    var everyi = '<div class="swiper-slide" data-id ="' + ii_arr[j]._id + '">' + '<div class="pic"><img src ="https:' + ii_arr[j].src + '"/></div>' + '<div class="info">' + '<p class="name">' + ii_arr[j].name + '</p>' + '<p class="saletip"><span>' + ii_arr[j].saletip + '</span></p>' + '<p class="price"><strong>' + ii_arr[j].price + '</strong>/' + ii_arr[i].unit + '</p>' + '</div>';
                ...
                for (var m = 0; m < $('.saletip span').length; m++) {
                    if ($($('.saletip span')[m]).text() == '') {
                        $($('.saletip span')[m]).css({ 'border': 'none' });
                    }
                }
        ...
        http.post('goodslist').then(function (res) {
            for (var i = 0; i < 11; i++) {
                var li = '<li data-id="' + res.data[i]._id + '">' + '<div class="lm_l">' + '<img src="' + res.data[i].SmallPic + '" alt="">' + '</div>' + '<div class="lm_r">' + '<p class="name">' + res.data[i].CommodityName + '</p>' + '<p>' + res.data[i].SubTitle + '</p>' + '<i>' + res.data[i].PromotionTag + '</i>' + '<p>' + '<span class="pr">' + res.data[i].SellPrice + '</span>' + '<span>' + res.data[i].OriginalPrice + '</span>' + '<sapn>' + res.data[i].Spec + '</sapn>' + '</p>' + '</div>' + '</li>';
...
require([
    ...
    'classify',
    'main'
], function ($, http, user, shop, classify, index) {
    $(function () {
